...
function VoteButtons({ count }: { count: number }) {
  return (
    <div className="flex items-center gap-0.5 shrink-0">
      <div className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/60 hover:text-orange-500 hover:bg-orange-50 transition-colors cursor-default">
        <ThumbsUp className="w-3.5 h-3.5" />
      </div>
      <span
        className={cn(
          'text-xs font-bold tabular-nums min-w-[1.5rem] text-center',
          count > 0 && 'text-orange-500',
      ...
      <div className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/60 hover:text-blue-500 hover:bg-blue-50 transition-colors cursor-default">
...
function ThreadRow({ thread, subforumId }: { thread: ThreadItem; subforumId: string }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2 border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
      {/* Votes — horizontal, Reddit-style */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {thread.pinned && (
            <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full shrink-0">
              📌 Fijado
            </span>
          )}
          ...
          <Link
            ...
            className="text-sm font-medium leading-snug hover:underline underline-offset-2 truncate"
          >
            {thread.title}
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap mt-0.5">
          <span className="font-medium text-foreground/60">@{thread.author.username}</span>
          ...
          {thread.tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 text-[10px]"
            >
      ...
      {/* Last comment — hidden on mobile */}
      <div className="hidden sm:flex flex-col items-end shrink-0 min-w-[8rem] max-w-[10rem] text-right">
        {thread.lastComment ? (
          <>
            <span className="text-xs font-medium text-foreground/60 truncate max-w-full">
          ...
          <span className="text-[10px] text-muted-foreground italic">Sin respuestas</span>
        )}
      </div>
    </div>
  );
...
function ThreadSkeleton() {
  return (
    <div className="flex items-center gap-3 px-3 py-2 border-b border-border last:border-0 animate-pulse">
      <div className="flex items-center gap-0.5 shrink-0">
